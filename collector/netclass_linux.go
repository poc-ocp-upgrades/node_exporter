...
func (c *netClassCollector) Update(ch chan<- prometheus.Metric) error {
	...
	for _, ifaceInfo := range netClass {
		upDesc := prometheus.NewDesc(prometheus.BuildFQName(namespace, c.subsystem, "up"), "Valid operstate for interface.", []string{"interface", "address", "broadcast", "duplex", "operstate", "ifalias"}, nil)
		upValue := 0.0
