	...
	for scanner.Scan() {
		...
		if kv := strings.Split(field, ":"); len(kv) == 2 {
			if id, err := strconv.ParseUint(kv[0], 10, 64); err == nil && kv[1] == "" {
				...
				ch <- prometheus.MustNewConstMetric(metric.desc, metric.valueType, value*metric.multiplier, device)
			} else if metric, ok := drbdStringPairMetrics[kv[0]]; ok {
				values := strings.Split(kv[1], "/")
				ch <- prometheus.MustNewConstMetric(metric.desc, prometheus.GaugeValue, metric.isOkay(values[0]), device, "local")
				ch <- prometheus.MustNewConstMetric(metric.desc, prometheus.GaugeValue, metric.isOkay(values[1]), device, "remote")
			} else if kv[0] == "cs" {
				var connected float64
				if kv[1] == "Connected" {
					connected = 1
				}
				ch <- prometheus.MustNewConstMetric(drbdConnected, prometheus.GaugeValue, connected, device)
			} else {
